...
def analyze_prompts(text):
    ...
    masked_text, found_identifiers = analyze_identifiers(text)
...
fucntion_call = gr.Interface(fn=analyze_prompts,
                    inputs=gr.Textbox(lines=10, placeholder="Prompt Here",label="Prompt"),
                    outputs=[gr.Textbox(placeholder="Masked response",label="Masked prompt"),
                             gr.Textbox(placeholder="Identifiers found ",label="Identfiers masked")],
                    allow_flagging="never")

fucntion_call.launch(share=False)
